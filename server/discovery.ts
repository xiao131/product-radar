...
export function persistDiscoveredSignals(
  ...
  db.transaction(() => {
    ...
    const insert = db.prepare(
      `INSERT INTO signals (
         ...
         discovery_run_id, auto_collected, created_at, updated_at
       ) VALUES (?, ?, ?, ?, ?, ?, 'NEW', NULL, ?, ?, ?, ?, ?, 1, ?, ?)`,
    );
    ...
    for (const input of uniqueInputs) {
      const existing = findExisting.get(input.fingerprint) as
        | {
            id: string;
            source_type: string;
            title: string;
  ...
  const fingerprints = uniqueInputs.map((input) => input.fingerprint);
  const signals = (
    db
      .prepare(
        `SELECT * FROM signals
  ...
  return {
    signals,
    inserted,
    reused: uniqueInputs.length - inserted,
    changedSignalIds,
  };
}
...
export async function runAutomaticDiscovery(
  ...
  const persisted = persistDiscoveredSignals(db, runId, collected.signals);
  ...
  const saved = persistDiscoveryCandidates(db, candidates, persisted.signals);
