...
下载带 UTF-8 BOM 的示例模板，兼容 Excel 和 WPS 的中文显示。模板包含一行有效
示例，正式导入前请替换或删除示例数据。
...
首次部署时使用至少 8 位的高强度密码生成密码哈希：

```bash
RADAR_ADMIN_PASSWORD='至少8位的高强度密码' npm run auth:hash
```
...
忘记密码时，可在服务器上直接重置为至少 8 位的新密码：

```bash
RADAR_ADMIN_PASSWORD='至少8位的新密码' npm run auth:reset
```
